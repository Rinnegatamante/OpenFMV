...
sequence *seg1000();
sequence *eval_from_ep9_to_ep11();
...
enum {
	...
	THE_PLOY, // Implemented
	THE_FOOL, // Implemented
	THE_DELIVERY, // Implemented
	THE_BRIBE, // Implemented
	THE_FRENZY, // Implemented
...
char *intimidate_him() { return game_strings[372]; }
char *keep_intimidating() { return game_strings[374]; }
char *open_yourself() { return game_strings[381]; }
char *use_force() { return game_strings[373]; }
char *force_him() { return game_strings[375]; }
char *stop_him() { return game_strings[376]; }
char *let_him() { return game_strings[377]; }
char *hit_him() { return game_strings[379]; }
char *reject() { return game_strings[393]; }
char *shoot_him() { return game_strings[387]; }
char *surrender() { return game_strings[388]; }
char *let_him_alive() { return game_strings[386]; }
char *tell_him() { return game_strings[390]; }
char *fuck_you() { return game_strings[391]; }
...
enum {
	...
	TORMENT_ESCALATION,
	TORMENT_DE_ESCALATION,
	ATMO_HOSPITAL,
	TORMENT_STOP_OR_STRANGLE,
	HOSPITAL_RECEPTION,
	...
	BOWLTEST_FAIL,
	REQUIEM_MAY,
	INTENTNESS_END_SHORT,
	PASSWORD,
	GETOUT,
	ATMO_HAINSWORTH_OFFICES,
	BOWLTEST,
	ATMO_VAN_STOP,
	LIFT_BOSSA_DEL_BREEZY_ENDLESS,
	REQUIEM_WIN,
	PAYBACK_STAIRWELL,
	BOWLTEST_RISER_SHORT,
	VO_BETHNAL_GREEN,
	ATMO_HAINSWORTH_PARKING_LOT,
	VO_WESTMINSTER_PLEASE,
	AWARENESS_MOOD,
	ATMO_VAN,
	HAINSWORTH_RETURN_LONG,
	DONE_STINGER,
	PASSWORD_ACCENT,
	HAINSWORTH_RETURN,
	REQUIEM_ARRIVAL,
...
void purge_from_ep7() {
	if (!fake_pass) {
		for (int i = FORGED; i <= INTENTNESS_MOOD; i++) {
			if (i != INTENTNESS_MOOD && i != AFTER_FIGHT_TRANS && i != POLICE_EVERYWHERE_2 && i != AFTER_FIGHT) {
				audio_sample_stop_and_free(mus[i]);
			}
		}
	}
...
void purge_from_ep9() {
	if (!fake_pass) {
		for (int i = TORMENT_SEBASTIAN; i <= ATMO_HEARTBEAT_SEB_SLOW; i++) {
			if (i != AWARENESS_DOWNLIFTER_9 && i != DESPERATION && i != WAKE_UP_CALL && i != DESPERATION_RISER && i != LONLINESS_RISER) {
				audio_sample_stop_and_free(mus[i]);
			}
		}
	}
}
void purge_from_ep10() {
	if (!fake_pass) {
		for (int i = REQUIEM_BROKEN; i <= ATMO_STREET; i++) {
			if (i != REQUIEM_MAY && i != PAYBACK_STAIRWELL && i != PAYBACK_FRONT && i != GETOUT && i != EXIT && i != ESCAPE) {
				audio_sample_stop_and_free(mus[i]);
			}
		}
	}
...
sequence *eval_payback_time() { return game_vars.payback_time ? eval_from_ep9_to_ep11() : seg1000(); }
// EPISODE 9B
...
sequence *eval_cross_exam2() { if (game_vars.cross_exam) { game_vars.from_ep9_to_ep11 = 1; return eval_from_ep9_to_ep11(); } return seg994(); }
sequence *eval_payback_time2() { if (game_vars.payback_time) { game_vars.from_ep9_to_ep11 = 1; return eval_from_ep9_to_ep11(); } game_vars.from_ep9_to_ep11 = 0; return seg1000(); }
...
// EPISODE 11
sequence *seg1103() { return &sequences[474]; }
sequence *seg1104() { return &sequences[475]; }
sequence *seg1101_2() { return &sequences[470]; }
sequence *eval_from_ep9_to_ep11() { purge_from_ep7(); purge_from_ep9(); purge_from_ep10(); exec_save(EPISODE_11); return game_vars.from_ep9_to_ep11 ? &sequences[469] : seg1101_2(); }
sequence *eval_called_parr2() { return game_vars.called_parr ? &sequences[471] : &sequences[472]; }
sequence *eval_aware_of_chest() { return game_vars.aware_of_chest ? &sequences[473] : seg1103(); }
sequence *eval_gone_to_woe3() { return game_vars.gone_to_woe ? &sequences[476] : &sequences[477]; }
sequence *seg1106() { game_vars.passed_by_seg1106 = 1; return &sequences[478]; }
sequence *seg1107() { return &sequences[479]; }
sequence *seg1117_1() { return &sequences[480]; }
sequence *seg1108() { return &sequences[481]; }
sequence *seg1117() { return &sequences[482]; }
sequence *seg1118_1122() { return &sequences[483]; }
sequence *seg1083_1084_alt() { return &sequences[484]; }
sequence *seg1068_1_1068_3_alt() { return &sequences[485]; }
sequence *seg1110() { game_vars.beat_up = 1; game_vars.passed_by_seg1110 = 1; return &sequences[486]; }
sequence *seg1111() { return &sequences[487]; }
sequence *seg1113() { return &sequences[488]; }
sequence *seg1112_2() { return &sequences[489]; }
sequence *seg1114_a() { return &sequences[490]; }
sequence *seg1114_b() { return &sequences[491]; }
sequence *seg1115() { return &sequences[492]; }
sequence *seg1116_a() { return &sequences[493]; }
sequence *seg1116_b() { return &sequences[494]; }
sequence *seg1109() { return &sequences[495]; }
sequence *seg1112() { return &sequences[496]; }
sequence *seg1112_1() { return &sequences[497]; }
sequence *eval_beat_up() { return game_vars.beat_up ? seg1113() : seg1112_2(); }
sequence *seg1123_1125() { return &sequences[498]; }
sequence *eval_from_ep7_to_ep11() { return game_vars.from_ep7_to_ep11 ? &sequences[499] : &sequences[500]; }
sequence *seg1127_1129() { return &sequences[501]; }
sequence *seg1130_1130_1() { return &sequences[502]; }
sequence *seg1131_1_1131_2() { return &sequences[503]; }
// END
sequence *endcredits() { return &sequences[504]; }
...
sequence *stop_atmo_reception() { audio_sample_stop_and_free(mus[ATMO_RECEPTION]); return NULL; }
sequence *stop_lonliness() { audio_sample_stop_and_free(mus[LONLINESS]); return NULL; }
sequence *stop_atmo_phone_vibra() { audio_sample_stop_and_free(mus[ATMO_PHONE_VIBRA]); return NULL; }
...
sequence *fade_hospital_reception3() { audio_sample_fade(mus[HOSPITAL_RECEPTION], 0.2525519f, 0.0f, 76875, 79000); return NULL; }
sequence *fade_hospital_reception4() { audio_sample_fade(mus[HOSPITAL_RECEPTION], 0.2525519f, 0.0f, 47000, 49300); return NULL; }
sequence *fade_hospital_reception5() { audio_sample_fade(mus[HOSPITAL_RECEPTION], 0.2525519f, 0.0f, 38542, 48542); return NULL; }
...
sequence *fade_hainsworth_return() { audio_sample_fade(mus[HAINSWORTH_RETURN], 0.09993204f, 0.05201785f, 0, 4625); return NULL; }
sequence *fade_password_accent() { audio_sample_fade(mus[PASSWORD_ACCENT], 0.1588699f, 0.0f, 1125, 3125); return NULL; }
sequence *stop_desperation() { audio_sample_stop_and_free(mus[DESPERATION]); return NULL; }
sequence *stop_password() { audio_sample_stop_and_free(mus[PASSWORD]); return NULL; }
sequence *stop_hainsworth_return() { audio_sample_stop_and_free(mus[HAINSWORTH_RETURN]); return NULL; }
sequence *stop_password_accent() { audio_sample_stop_and_free(mus[PASSWORD_ACCENT]); return NULL; }
sequence *stop_atmo_hainsworth_offices() { audio_sample_stop_and_free(mus[ATMO_HAINSWORTH_OFFICES]); return NULL; }
sequence *stop_atmo_hainsworth_corridor() { audio_sample_stop_and_free(mus[ATMO_HAINSWORTH_CORRIDOR]); return NULL; }
sequence *stop_atmo_hainsworth_parking() { audio_sample_stop_and_free(mus[ATMO_HAINSWORTH_PARKING_LOT]); return NULL; }
sequence *stop_wake_up_call() { audio_sample_stop_and_free(mus[WAKE_UP_CALL]); return NULL; }
sequence *stop_lift_bossa_del_breezy() { audio_sample_stop_and_free(mus[LIFT_BOSSA_DEL_BREEZY_ENDLESS]); return NULL; }
sequence *stop_intentness_mood() { audio_sample_stop_and_free(mus[INTENTNESS_MOOD]); return NULL; }
sequence *stop_intentness() { audio_sample_stop_and_free(mus[INTENTNESS]); return NULL; }
sequence *stop_atmo_van() { audio_sample_stop_and_free(mus[ATMO_VAN]); return NULL; }
sequence *stop_requiem_arrival() { audio_sample_stop_and_free(mus[REQUIEM_ARRIVAL]); return NULL; }
sequence *change_lift_bossa_del_breezy() { audio_sample_set_volume(mus[LIFT_BOSSA_DEL_BREEZY_ENDLESS], 0.0297619f); return NULL; }
sequence *change_lift_bossa_del_breezy2() { audio_sample_set_volume(mus[LIFT_BOSSA_DEL_BREEZY_ENDLESS], 0.07271375f); return NULL; }
sequence *change_lift_bossa_del_breezy3() { audio_sample_set_volume(mus[LIFT_BOSSA_DEL_BREEZY_ENDLESS], 0.03944742f); return NULL; }
sequence *change_lift_bossa_del_breezy4() { audio_sample_set_volume(mus[LIFT_BOSSA_DEL_BREEZY_ENDLESS], 0.04029473f); return NULL; }
sequence *change_lift_bossa_del_breezy5() { audio_sample_set_volume(mus[LIFT_BOSSA_DEL_BREEZY_ENDLESS], 0.07976449f); return NULL; }
sequence *change_lift_bossa_del_breezy6() { audio_sample_set_volume(mus[LIFT_BOSSA_DEL_BREEZY_ENDLESS], 0.00295858f); return NULL; }
sequence *change_lift_bossa_del_breezy7() { audio_sample_set_volume(mus[LIFT_BOSSA_DEL_BREEZY_ENDLESS], 0.0f); return NULL; }
sequence *change_requiem_win() { audio_sample_set_volume(mus[REQUIEM_WIN], 0.1353797f); return NULL; }
...
sequence *stop_atmo_parr_house() { audio_sample_stop_and_free(mus[ATMO_PARR_HOUSE]); return NULL; }
...
sequence *fade_showdown() { audio_sample_fade(mus[SHOWDOWN], 0.08878937f, 0.0f, 9708, 14708); return NULL; }
sequence *fade_showdown2() { audio_sample_fade(mus[SHOWDOWN], 0.08878937f, 0.0f, 28625, 42625); return NULL; }
sequence *fade_showdown3() { audio_sample_fade(mus[SHOWDOWN], 0.08878937f, 0.0f, 1083, 6083); return NULL; }
sequence *fade_showdown4() { audio_sample_fade(mus[SHOWDOWN], 0.08878937f, 0.0f, 17667, 22667); return NULL; }
sequence *fade_showdown5() { audio_sample_fade(mus[SHOWDOWN], 0.08878937f, 0.0f, 42, 10000); return NULL; }
sequence *fade_requiem_where() { audio_sample_fade(mus[REQUIEM_WHERE], 0.04485771f, 0.08927047f, 43625, 67042); return NULL; }
sequence *stop_lonliness_riser() { audio_sample_stop_and_free(mus[LONLINESS_RISER]); return NULL; }
sequence *stop_payback_front() { audio_sample_stop_and_free(mus[PAYBACK_FRONT]); return NULL; }
sequence *stop_payback_stairwell() { audio_sample_stop_and_free(mus[PAYBACK_STAIRWELL]); return NULL; }
sequence *stop_exit() { audio_sample_stop_and_free(mus[EXIT]); return NULL; }
sequence *stop_getout() { audio_sample_stop_and_free(mus[GETOUT]); return NULL; }
sequence *stop_after_fight() { audio_sample_stop_and_free(mus[AFTER_FIGHT]); return NULL; }
sequence *stop_showdown() { audio_sample_stop_and_free(mus[SHOWDOWN]); return NULL; }
...
sequence *unlock_the_delivery() { unlock_ending(THE_DELIVERY); return NULL; }
sequence *unlock_the_frenzy() { unlock_ending(THE_FRENZY); return NULL; }
sequence *unlock_the_bribe() { unlock_ending(THE_BRIBE); return NULL; }
...
void fill_events() {
	...
	install_timed_event(&sequences[292], 9167, 10750, EVENT_ONESHOT, fade_matt_alone2);
	...
	// EPISODE 9A
	// seg901_1_901_2
	install_timed_event(&sequences[317], 7750, 0, EVENT_ONESHOT, start_atmo_reception);
	install_timed_event(&sequences[317], 7750, 22000, EVENT_DURATION, fade_police_everywhere_p2);
	install_timed_event(&sequences[317], 22000, 0, EVENT_ONESHOT, stop_police_everywhere_p2);
	// seg902_b (empty)
	// seg902_a (empty)
	// seg903_a (empty)
	// seg903_b (empty)
	// seg904_a (empty)
	// seg904_b_905_1
	install_timed_event(&sequences[323], 14250, 0, EVENT_ONESHOT, start_hospital_reception);
	// seg906_a (empty)
	// seg906_b (empty)
	// seg907_1 (empty)
	// seg946_1 (empty)
	// seg905_2
	install_timed_event(&sequences[328], 10125, 0, EVENT_ONESHOT, start_hospital_reception);
	// seg907_2 (empty)
	// seg908_a_909_913
	install_timed_event(&sequences[330], 46542, 56542, EVENT_DURATION, fade_hospital_reception);
	install_timed_event(&sequences[330], 56542, 0, EVENT_ONESHOT, stop_hospital_reception);
	// seg908_c (empty)
	// seg908_b (empty)
	// seg910 (empty)
	// seg912_913
	install_timed_event(&sequences[334], 27250, 37250, EVENT_DURATION, fade_hospital_reception2);
	install_timed_event(&sequences[334], 37250, 0, EVENT_ONESHOT, stop_hospital_reception);
	// seg915
	install_timed_event(&sequences[335], 0, 0, EVENT_ONESHOT, start_hospital_reception);
	install_timed_event(&sequences[335], 42, 0, EVENT_ONESHOT, stop_atmo_reception);
	install_timed_event(&sequences[335], 69667, 0, EVENT_ONESHOT, start_atmo_hospital);
	install_timed_event(&sequences[335], 69708, 0, EVENT_ONESHOT, start_atmo_hb_seb_slow);
	install_timed_event(&sequences[335], 76875, 0, EVENT_ONESHOT, start_torment_seb);
	install_timed_event(&sequences[335], 76875, 79000, EVENT_DURATION, fade_hospital_reception3);
	install_timed_event(&sequences[335], 79000, 0, EVENT_ONESHOT, stop_hospital_reception);
	// seg916 (empty)
	// seg917_b (empty)
	// seg917_a (empty)
	// seg918
	install_timed_event(&sequences[339], 2458, 0, EVENT_ONESHOT, stop_atmo_hb_seb_slow);
	install_timed_event(&sequences[339], 2875, 0, EVENT_ONESHOT, start_atmo_hb_seb_fast);
	install_timed_event(&sequences[339], 31333, 0, EVENT_ONESHOT, start_torment_accent);
	// seg919 (empty)
	// seg920 (empty)
	// seg914
	install_timed_event(&sequences[342], 42, 0, EVENT_ONESHOT, stop_atmo_reception);
	install_timed_event(&sequences[342], 37667, 0, EVENT_ONESHOT, start_atmo_hospital);
	install_timed_event(&sequences[342], 39667, 0, EVENT_ONESHOT, start_torment_seb);
	install_timed_event(&sequences[342], 39667, 0, EVENT_ONESHOT, start_atmo_hb_seb_slow);
	install_timed_event(&sequences[342], 47000, 49300, EVENT_DURATION, fade_hospital_reception4);
	install_timed_event(&sequences[342], 49300, 0, EVENT_ONESHOT, stop_hospital_reception);
	// seg911_913
	...
	// seg921 (empty)
	// seg922
	install_timed_event(&sequences[347], 34375, 0, EVENT_ONESHOT, stop_atmo_hb_seb_fast);
	install_timed_event(&sequences[347], 35000, 0, EVENT_ONESHOT, start_torment_de_escalation);
	install_timed_event(&sequences[347], 36167, 0, EVENT_ONESHOT, start_atmo_hb_seb_slow);
	install_timed_event(&sequences[347], 36167, 40000, EVENT_DURATION, fade_torment_seb);
	install_timed_event(&sequences[347], 40000, 0, EVENT_ONESHOT, stop_torment_seb);
	// seg923 (empty)
	// seg924 (empty)
	// seg927 (empty)
	// seg928
	install_timed_event(&sequences[351], 1333, 0, EVENT_ONESHOT, stop_atmo_hb_seb_fast);
	install_timed_event(&sequences[351], 3083, 0, EVENT_ONESHOT, start_atmo_hb_seb_slow);
	// seg928_939
	install_timed_event(&sequences[352], 0, 0, EVENT_ONESHOT, start_de_escalation_no_boom);
	install_timed_event(&sequences[352], 0, 5000, EVENT_DURATION, fade_torment_seb2);
	install_timed_event(&sequences[352], 5000, 0, EVENT_ONESHOT, stop_torment_seb);
	install_timed_event(&sequences[352], 6000, 0, EVENT_ONESHOT, change_hb_seb_slow);
	// seg929
	install_timed_event(&sequences[353], 0, 0, EVENT_ONESHOT, start_torment_accent);
	// seg930
	...
	install_timed_event(&sequences[356], 0, 0, EVENT_ONESHOT, start_torment_escalation);
	install_timed_event(&sequences[356], 0, 5000, EVENT_DURATION, fade_torment_seb2);
	install_timed_event(&sequences[356], 5000, 0, EVENT_ONESHOT, stop_torment_seb);
	install_timed_event(&sequences[356], 61458, 0, EVENT_ONESHOT, stop_atmo_hb_seb_slow);
	install_timed_event(&sequences[356], 61875, 0, EVENT_ONESHOT, start_atmo_hb_may_ascending);
	install_timed_event(&sequences[356], 70083, 0, EVENT_ONESHOT, stop_atmo_hb_may_ascending);
	install_timed_event(&sequences[356], 73250, 0, EVENT_ONESHOT, start_atmo_hb_seb_fast2);
	install_timed_event(&sequences[356], 78250, 0, EVENT_ONESHOT, start_lonliness);
	// seg929_939
	install_timed_event(&sequences[357], 0, 0, EVENT_ONESHOT, start_de_escalation_no_boom);
	install_timed_event(&sequences[357], 0, 5000, EVENT_DURATION, fade_torment_seb2);
	install_timed_event(&sequences[357], 5000, 0, EVENT_ONESHOT, stop_torment_seb);
	install_timed_event(&sequences[357], 6000, 0, EVENT_ONESHOT, change_hb_seb_slow);
	// seg933_2_937_a_938
	install_timed_event(&sequences[358], 17750, 0, EVENT_ONESHOT, stop_atmo_hb_seb_slow);
	install_timed_event(&sequences[358], 18167, 0, EVENT_ONESHOT, start_atmo_hb_may_ascending);
	install_timed_event(&sequences[358], 27334, 0, EVENT_ONESHOT, stop_atmo_hb_may_ascending);
	install_timed_event(&sequences[358], 29708, 0, EVENT_ONESHOT, start_atmo_hb_seb_fast2);
	install_timed_event(&sequences[358], 34542, 0, EVENT_ONESHOT, start_lonliness);
	// seg936 (empty)
	// seg937_a_938
	install_timed_event(&sequences[360], 52125, 0, EVENT_ONESHOT, stop_atmo_hb_seb_fast);
	install_timed_event(&sequences[360], 52125, 0, EVENT_ONESHOT, stop_atmo_hospital);
	// seg937_b_938
	install_timed_event(&sequences[361], 42, 0, EVENT_ONESHOT, stop_atmo_hospital);
	install_timed_event(&sequences[361], 26833, 0, EVENT_ONESHOT, stop_atmo_hb_seb_fast);
	// seg939
	install_timed_event(&sequences[362], 18208, 0, EVENT_ONESHOT, start_awareness_downlifter_9);
	install_timed_event(&sequences[362], 21125, 0, EVENT_ONESHOT, start_tenseness_long);
	install_timed_event(&sequences[362], 129208, 0, EVENT_ONESHOT, start_torment_de_escalation);
	// seg945
	install_timed_event(&sequences[363], 11000, 0, EVENT_ONESHOT, stop_lonliness);
	install_timed_event(&sequences[363], 11000, 0, EVENT_ONESHOT, start_lonliness_riser);
	install_timed_event(&sequences[363], 12750, 0, EVENT_ONESHOT, start_wake_up);
	install_timed_event(&sequences[363], 16167, 0, EVENT_ONESHOT, start_atmo_phone_vibra);
	install_timed_event(&sequences[363], 16167, 24167, EVENT_DURATION, fade_atmo_phone_vibra);
	install_timed_event(&sequences[363], 39417, 0, EVENT_ONESHOT, stop_atmo_phone_vibra);
	...
	// EPISODE 10
	// seg1000
	install_timed_event(&sequences[412], 0, 0, EVENT_ONESHOT, stop_desperation);
	install_timed_event(&sequences[412], 0, 0, EVENT_ONESHOT, start_lift_bossa_del_breezy);
	install_timed_event(&sequences[412], 1000, 0, EVENT_ONESHOT, stop_wake_up_call);
	install_timed_event(&sequences[412], 1958, 0, EVENT_ONESHOT, start_intentness);
	install_timed_event(&sequences[412], 1958, 4000, EVENT_DURATION, fade_desperation_riser3);
	install_timed_event(&sequences[412], 4000, 0, EVENT_ONESHOT, stop_desperation_riser);
	install_timed_event(&sequences[412], 4000, 7958, EVENT_ONESHOT, fade_intentness_mood2);
	install_timed_event(&sequences[412], 7958, 0, EVENT_ONESHOT, stop_intentness_mood);
	// seg1001_1003
	install_timed_event(&sequences[413], 6375, 0, EVENT_ONESHOT, stop_desperation);
	// seg1004_k (empty)
	// seg1006
	install_timed_event(&sequences[415], 0, 10000, EVENT_ONESHOT, fade_intentness);
	install_timed_event(&sequences[415], 10000, 0, EVENT_ONESHOT, stop_intentness);
	install_timed_event(&sequences[415], 27625, 0, EVENT_ONESHOT, change_lift_bossa_del_breezy);
	// seg1007
	install_timed_event(&sequences[416], 0, 10000, EVENT_ONESHOT, fade_intentness);
	install_timed_event(&sequences[416], 10000, 0, EVENT_ONESHOT, stop_intentness);
	// seg1020_1
	install_timed_event(&sequences[417], 0, 10000, EVENT_ONESHOT, fade_intentness);
	install_timed_event(&sequences[417], 10000, 0, EVENT_ONESHOT, stop_intentness);
	// seg1021_1
	install_timed_event(&sequences[418], 0, 0, EVENT_ONESHOT, change_lift_bossa_del_breezy2);
	// seg1021_2
	install_timed_event(&sequences[419], 0, 0, EVENT_ONESHOT, change_lift_bossa_del_breezy2);
	// seg1021_3 (empty)
	// seg1022_1023_b (empty)
	// seg1026_b_1025_1027_tap
	install_timed_event(&sequences[422], 4250, 0, EVENT_ONESHOT, change_lift_bossa_del_breezy3);
	// seg1028_1033_1036_k
	install_timed_event(&sequences[423], 6500, 0, EVENT_ONESHOT, change_lift_bossa_del_breezy4);
	// seg1024_1027_tap
	install_timed_event(&sequences[424], 6500, 0, EVENT_ONESHOT, change_lift_bossa_del_breezy4);
	// seg1008_a (empty)
	// seg1008_b
	install_timed_event(&sequences[426], 10125, 0, EVENT_ONESHOT, change_lift_bossa_del_breezy);
	// seg1013_b (empty)
	// seg1014_a
	install_timed_event(&sequences[428], 16667, 0, EVENT_ONESHOT, change_lift_bossa_del_breezy);
	// seg1014_b_1015
	install_timed_event(&sequences[429], 4542, 0, EVENT_ONESHOT, start_payback_front);
	// seg1013_a_1016_b
	install_timed_event(&sequences[430], 40292, 0, EVENT_ONESHOT, change_lift_bossa_del_breezy);
	// seg1037
	install_timed_event(&sequences[431], 1125, 0, EVENT_ONESHOT, start_password_accent);
	install_timed_event(&sequences[431], 1125, 3125, EVENT_ONESHOT, fade_password_accent);
	install_timed_event(&sequences[431], 3125, 0, EVENT_ONESHOT, stop_password_accent);
	// seg1041
	install_timed_event(&sequences[432], 4958, 0, EVENT_ONESHOT, stop_atmo_hainsworth_offices);
	install_timed_event(&sequences[432], 4958, 0, EVENT_ONESHOT, start_atmo_hainsworth_corridor);
	// seg1046_k
	install_timed_event(&sequences[433], 0, 0, EVENT_ONESHOT, start_password);
	// seg1042
	install_timed_event(&sequences[434], 6583, 0, EVENT_ONESHOT, start_escape);
	install_timed_event(&sequences[434], 7667, 0, EVENT_ONESHOT, stop_atmo_hainsworth_corridor);
	// seg1043_1044 (empty)
	// seg1045_1044
	install_timed_event(&sequences[436], 2167, 0, EVENT_ONESHOT, stop_atmo_hainsworth_corridor);
	install_timed_event(&sequences[436], 3667, 0, EVENT_ONESHOT, start_payback_stairwell);
	// seg1046
	install_timed_event(&sequences[437], 0, 0, EVENT_ONESHOT, start_lift_bossa_del_breezy);
	install_timed_event(&sequences[437], 10417, 0, EVENT_ONESHOT, stop_password);
	install_timed_event(&sequences[437], 10417, 0, EVENT_ONESHOT, start_password_accent);
	install_timed_event(&sequences[437], 13625, 0, EVENT_ONESHOT, change_lift_bossa_del_breezy4);
	install_timed_event(&sequences[437], 14333, 0, EVENT_ONESHOT, stop_atmo_hainsworth_offices);
	// seg1047_a_1045
	install_timed_event(&sequences[438], 4125, 0, EVENT_ONESHOT, stop_lift_bossa_del_breezy);
	// seg1048
	install_timed_event(&sequences[439], 0, 0, EVENT_ONESHOT, stop_lift_bossa_del_breezy);
	install_timed_event(&sequences[439], 6000, 0, EVENT_ONESHOT, start_hainsworth_return);
	// seg1047_b
	install_timed_event(&sequences[440], 2583, 0, EVENT_ONESHOT, change_lift_bossa_del_breezy5);
	// seg1049_a (empty)
	// seg1049_b (empty)
	// seg1050_b
	install_timed_event(&sequences[443], 958, 0, EVENT_ONESHOT, change_lift_bossa_del_breezy3);
	install_timed_event(&sequences[443], 9375, 0, EVENT_ONESHOT, change_lift_bossa_del_breezy6);
	// seg1050_a_1051_1052_b_1054_b
	install_timed_event(&sequences[444], 0, 0, EVENT_ONESHOT, change_lift_bossa_del_breezy7);
	install_timed_event(&sequences[444], 0, 0, EVENT_ONESHOT, start_atmo_hall);
	install_timed_event(&sequences[444], 1042, 0, EVENT_ONESHOT, change_lift_bossa_del_breezy4);
	install_timed_event(&sequences[444], 7333, 0, EVENT_ONESHOT, start_atmo_hainsworth_parking);
	install_timed_event(&sequences[444], 7333, 0, EVENT_ONESHOT, stop_atmo_hall);
	install_timed_event(&sequences[444], 7458, 0, EVENT_ONESHOT, stop_lift_bossa_del_breezy);
	install_timed_event(&sequences[444], 20708, 0, EVENT_ONESHOT, start_hainsworth_return_long);
	// seg1049_a (empty)
	// seg1057
	install_timed_event(&sequences[446], 0, 4625, EVENT_DURATION, fade_hainsworth_return);
	install_timed_event(&sequences[446], 4625, 0, EVENT_ONESHOT, stop_atmo_hainsworth_parking);
	install_timed_event(&sequences[446], 5458, 0, EVENT_ONESHOT, stop_hainsworth_return);
	install_timed_event(&sequences[446], 15875, 0, EVENT_ONESHOT, start_exit);
	// seg1058 (empty)
	// seg1059_1_1060
	install_timed_event(&sequences[448], 0, 0, EVENT_ONESHOT, start_atmo_van);
	install_timed_event(&sequences[448], 0, 0, EVENT_ONESHOT, stop_atmo_hainsworth_parking);
	// seg1061_a_1069_1069_1
	install_timed_event(&sequences[449], 25625, 0, EVENT_ONESHOT, start_atmo_van_stop);
	install_timed_event(&sequences[449], 25625, 0, EVENT_ONESHOT, change_atmo_van);
	install_timed_event(&sequences[449], 45583, 0, EVENT_ONESHOT, start_tenseness10);
	install_timed_event(&sequences[449], 72125, 0, EVENT_ONESHOT, stop_atmo_van);
	install_timed_event(&sequences[449], 72125, 0, EVENT_ONESHOT, change_atmo_van_stop);
	// seg1063_1078
	install_timed_event(&sequences[450], 0, 0, EVENT_ONESHOT, stop_atmo_van);
	install_timed_event(&sequences[450], 0, 0, EVENT_ONESHOT, start_atmo_van_stop);
	install_timed_event(&sequences[450], 13750, 0, EVENT_ONESHOT, start_atmo_street);
	// seg1067_3
	install_timed_event(&sequences[451], 42, 0, EVENT_ONESHOT, stop_atmo_van);
	// seg1068_1_1068_3
	install_timed_event(&sequences[452], 0, 0, EVENT_ONESHOT, start_requiem_arrival);
	install_timed_event(&sequences[452], 97250, 107250, EVENT_DURATION, fade_requiem_arrival);
	install_timed_event(&sequences[452], 107250, 0, EVENT_ONESHOT, stop_requiem_arrival);
	install_timed_event(&sequences[452], 110750, 0, EVENT_ONESHOT, start_requiem_may);
	// seg1069_3
	install_timed_event(&sequences[453], 31583, 0, EVENT_ONESHOT, start_atmo_parr_house);
	install_timed_event(&sequences[453], 31667, 41667, EVENT_DURATION, fade_tenseness10);
	install_timed_event(&sequences[453], 41667, 0, EVENT_ONESHOT, stop_tenseness10);
	// seg1070_a
	install_timed_event(&sequences[454], 7708, 0, EVENT_ONESHOT, start_bowltest);
	// seg1070_b
	install_timed_event(&sequences[455], 20042, 0, EVENT_ONESHOT, start_bowltest);
	// seg1072_a (empty)
	// seg1072_b_1073_1_1073_2
	install_timed_event(&sequences[457], 41375, 0, EVENT_ONESHOT, start_awareness_downlifter_9_2);
	install_timed_event(&sequences[457], 45083, 0, EVENT_ONESHOT, start_bowltest_riser_short);
	install_timed_event(&sequences[457], 45125, 0, EVENT_ONESHOT, start_bowltest_fail);
	install_timed_event(&sequences[457], 45125, 54125, EVENT_DURATION, fade_bowltest);
	install_timed_event(&sequences[457], 54125, 0, EVENT_ONESHOT, stop_bowltest);
	install_timed_event(&sequences[457], 87833, 0, EVENT_ONESHOT, stop_atmo_parr_house);
	install_timed_event(&sequences[457], 88125, 0, EVENT_ONESHOT, start_requiem_broken);
	install_timed_event(&sequences[457], 100000, 0, EVENT_ONESHOT, unlock_the_fool);
	// seg1075_1
	install_timed_event(&sequences[458], 3000, 0, EVENT_ONESHOT, unlock_the_ploy);
	// seg1079
	install_timed_event(&sequences[459], 12417, 0, EVENT_ONESHOT, start_vo_westminster_or_bethnal);
	install_timed_event(&sequences[459], 12708, 0, EVENT_ONESHOT, maybe_start_intentess_end);
	install_timed_event(&sequences[459], 16583, 0, EVENT_ONESHOT, stop_atmo_street);
	// seg1075_4 (empty)
	// seg1075_5 (empty)
	// seg1075_3 (empty)
	// seg1083_1084
	install_timed_event(&sequences[463], 76917, 0, EVENT_ONESHOT, stop_intentness);
	// seg1075_6_1075_2
	install_timed_event(&sequences[464], 20875, 0, EVENT_ONESHOT, start_done_stinger);
	install_timed_event(&sequences[464], 22625, 0, EVENT_ONESHOT, stop_bowltest);
	install_timed_event(&sequences[464], 40583, 0, EVENT_ONESHOT, stop_atmo_parr_house);
	install_timed_event(&sequences[464], 40667, 0, EVENT_ONESHOT, start_atmo_parr_end);
	install_timed_event(&sequences[464], 41417, 0, EVENT_ONESHOT, start_requiem_win);
	install_timed_event(&sequences[464], 71500, 0, EVENT_ONESHOT, change_requiem_win);
	// seg1076_a
	install_timed_event(&sequences[465], 20667, 0, EVENT_ONESHOT, stop_atmo_parr_end);
	// seg1076_b
	install_timed_event(&sequences[466], 18583, 0, EVENT_ONESHOT, stop_atmo_parr_end);
	// seg1081_a_1069_2
	install_timed_event(&sequences[467], 0, 0, EVENT_ONESHOT, start_tenseness10_2);
	install_timed_event(&sequences[467], 9625, 0, EVENT_ONESHOT, stop_atmo_street);
	// seg1081_b
	install_timed_event(&sequences[468], 0, 0, EVENT_ONESHOT, stop_atmo_street);
	// EPISODE 11
	// seg1101_1
	install_timed_event(&sequences[469], 0, 0, EVENT_ONESHOT, stop_wake_up_call);
	install_timed_event(&sequences[469], 1000, 0, EVENT_ONESHOT, stop_desperation_riser);
	install_timed_event(&sequences[469], 1000, 0, EVENT_ONESHOT, stop_auction_guy);
	install_timed_event(&sequences[469], 1000, 0, EVENT_ONESHOT, stop_after_fight);
	install_timed_event(&sequences[469], 2083, 0, EVENT_ONESHOT, start_intentness_11);
	install_timed_event(&sequences[469], 2083, 6000, EVENT_DURATION, fade_lonliness_riser);
	install_timed_event(&sequences[469], 6000, 0, EVENT_ONESHOT, stop_lonliness_riser);
	// seg1101_2
	install_timed_event(&sequences[470], 30000, 0, EVENT_ONESHOT, stop_payback_front);
	install_timed_event(&sequences[470], 31000, 0, EVENT_ONESHOT, stop_exit);
	install_timed_event(&sequences[470], 31000, 0, EVENT_ONESHOT, stop_getout);
	install_timed_event(&sequences[470], 31000, 0, EVENT_ONESHOT, stop_escape);
	install_timed_event(&sequences[470], 31000, 37125, EVENT_DURATION, fade_intentness_11);
	install_timed_event(&sequences[470], 37125, 0, EVENT_ONESHOT, stop_intentness_11);
	install_timed_event(&sequences[470], 37125, 0, EVENT_ONESHOT, start_atmo_parr);
	install_timed_event(&sequences[470], 37125, 52958, EVENT_DURATION, fade_payback_stairwell);
	install_timed_event(&sequences[470], 52958, 0, EVENT_ONESHOT, stop_payback_stairwell);
	// seg1102_a (empty)
	// seg1102_b1 (empty)
	// seg1102_b2 (empty)
	// seg1103
	install_timed_event(&sequences[474], 0, 0, EVENT_ONESHOT, start_showdown);
	// seg1104 (empty)
	// seg1105 (empty)
	// seg1105_2 (empty)
	// seg1106
	install_timed_event(&sequences[478], 5750, 0, EVENT_ONESHOT, start_showdown_safe_accent);
	// seg1107
	install_timed_event(&sequences[479], 9708, 0, EVENT_ONESHOT, start_showdown_riser_more);
	install_timed_event(&sequences[479], 9708, 14708, EVENT_DURATION, fade_showdown);
	install_timed_event(&sequences[479], 14708, 0, EVENT_ONESHOT, stop_showdown);
	// seg1117_1 (empty)
	// seg1108 (empty)
	// seg1117 (empty)
	// seg1118_1122
	install_timed_event(&sequences[483], 27000, 0, EVENT_ONESHOT, unlock_the_delivery);
	install_timed_event(&sequences[483], 28625, 42625, EVENT_DURATION, fade_showdown2);
	install_timed_event(&sequences[483], 42625, 0, EVENT_ONESHOT, stop_showdown);
	install_timed_event(&sequences[483], 43625, 0, EVENT_ONESHOT, start_requiem_where);
	install_timed_event(&sequences[483], 43625, 67042, EVENT_DURATION, fade_requiem_where);
	install_timed_event(&sequences[483], 67042, 0, EVENT_ONESHOT, stop_atmo_parr);
	// seg1083_1084_alt
	install_timed_event(&sequences[484], 0, 0, EVENT_ONESHOT, start_intentness_alt);
	// seg1068_1_1068_3_alt
	install_timed_event(&sequences[485], 0, 0, EVENT_ONESHOT, start_requiem_after_where);
	install_timed_event(&sequences[485], 68042, 0, EVENT_ONESHOT, start_requiem_may2);
	// seg1110
	install_timed_event(&sequences[486], 0, 0, EVENT_ONESHOT, start_showdown_violence);
	install_timed_event(&sequences[486], 35875, 0, EVENT_ONESHOT, start_showdown_safe_accent2);
	// seg1111
	install_timed_event(&sequences[487], 1083, 0, EVENT_ONESHOT, maybe_start_showdown_riser_more);
	install_timed_event(&sequences[487], 1083, 6083, EVENT_DURATION, fade_showdown3);
	install_timed_event(&sequences[487], 6083, 0, EVENT_ONESHOT, stop_showdown);
	// seg1113
	install_timed_event(&sequences[488], 10000, 0, EVENT_ONESHOT, unlock_the_frenzy);
	install_timed_event(&sequences[488], 10375, 0, EVENT_ONESHOT, start_requiem_parr);
	install_timed_event(&sequences[488], 58958, 62958, EVENT_DURATION, fade_requiem_parr);
	install_timed_event(&sequences[488], 99417, 0, EVENT_ONESHOT, stop_atmo_parr);
	// seg1112_2 (empty)
	// seg1114_a (empty)
	// seg1114_b (empty)
	// seg1115
	install_timed_event(&sequences[492], 5542, 0, EVENT_ONESHOT, start_requiem_matt);
	install_timed_event(&sequences[492], 19083, 0, EVENT_ONESHOT, maybe_seg1116_a);
	// seg1116_a (empty)
	// seg1116_b (empty)
	// seg1109 (empty)
	// seg1112
	install_timed_event(&sequences[496], 17667, 0, EVENT_ONESHOT, start_showdown_riser_more2);
	install_timed_event(&sequences[496], 17667, 22667, EVENT_ONESHOT, fade_showdown4);
	install_timed_event(&sequences[496], 22667, 0, EVENT_ONESHOT, stop_showdown);
	// seg1112_1 (empty)
	// seg1123_1125
	install_timed_event(&sequences[498], 42, 10000, EVENT_DURATION, fade_showdown5);
	install_timed_event(&sequences[498], 10000, 0, EVENT_ONESHOT, unlock_the_bribe);
	install_timed_event(&sequences[498], 11042, 0, EVENT_ONESHOT, stop_showdown);
	install_timed_event(&sequences[498], 34333, 0, EVENT_ONESHOT, start_wanted);
	install_timed_event(&sequences[498], 42333, 0, EVENT_ONESHOT, stop_atmo_parr);
	// seg1126_2 (empty)
	// seg1126_1 (empty)
	// seg1127_1129
	install_timed_event(&sequences[501], 53917, 0, EVENT_ONESHOT, start_requiem_arrival2);
	// seg1130_1130_1
	install_timed_event(&sequences[502], 45375, 0, EVENT_ONESHOT, start_requiem_may3);
	// seg1131_1_1131_2
	install_timed_event(&sequences[503], 27292, 0, EVENT_ONESHOT, start_requiem_sorry);
	// END
	// endcredits (empty)
}
...
void fill_sequences() {
	...
	fill_sequence("seg739_b", &sequences[297], eval_from_ep9_to_ep11, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg739_b (END OF EPISODE -> 11)
	...
	fill_sequence("seg945", &sequences[363], eval_payback_time, maybe_hainsworths, parr, NULL, seg1000, eval_from_ep9_to_ep11, NULL, 64292, 70250, 70250); // seg945 (END OF EPISODE -> 11/10)
	// EPISODE 9B
	...
	fill_sequence("seg1043_1044", &sequences[435], eval_from_ep9_to_ep11, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1043_1044 (END OF EPISODE -> 11)
	fill_sequence("seg1045_1044", &sequences[436], eval_from_ep9_to_ep11, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1045_1044 (END OF EPISODE -> 11)
	...
	fill_sequence("seg1050_b", &sequences[443], eval_from_ep9_to_ep11, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1050_b (END OF EPISODE -> 11)
	...
	fill_sequence("seg1057", &sequences[446], eval_from_ep9_to_ep11, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1057 (END OF EPISODE -> 11)
	...
	fill_sequence("seg1068_1_1068_3", &sequences[452], endcredits, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1068_1_1068_3 (END OF EPISODE -> END)
	...
	fill_sequence("seg1083_1084", &sequences[463], endcredits, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1083_1084 (END OF EPISODE -> END)
	fill_sequence("seg1075_6_1075_2", &sequences[464], eval_may_stabbed, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1075_6_1075_2
	fill_sequence("seg1076_a", &sequences[465], endcredits, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1076_a (END OF EPISODE -> END)
	fill_sequence("seg1076_b", &sequences[466], endcredits, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1076_b (END OF EPISODE -> END)
	...
	// EPISODE 11
	fill_sequence("seg1101_1", &sequences[469], seg1101_2, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1101_1
	fill_sequence("seg1101_2", &sequences[470], eval_called_parr2, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1101_2
	fill_sequence("seg1102_a", &sequences[471], seg1103, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1102_a
	fill_sequence("seg1102_b1", &sequences[472], eval_aware_of_chest, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1102_b1
	fill_sequence("seg1102_b2", &sequences[473], seg1103, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1102_b2
	fill_sequence("seg1103", &sequences[474], seg1108, intimidate_him, use_force, NULL, seg1104, seg1108, NULL, 46958, 51000, 48958); // seg1103
	fill_sequence("seg1104", &sequences[475], seg1108, keep_intimidating, force_him, NULL, eval_gone_to_woe3, seg1108, NULL, 9958, 13750, 12042); // seg1104
	fill_sequence("seg1105", &sequences[476], seg1106, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1105
	fill_sequence("seg1105_2", &sequences[477], seg1106, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1105_2
	fill_sequence("seg1106", &sequences[478], seg1111, stop_him, let_him, NULL, seg1107, seg1111, NULL, 5750, 10708, 10708); // seg1106
	fill_sequence("seg1107", &sequences[479], seg1117_1, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1107
	fill_sequence("seg1117_1", &sequences[480], seg1117, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1117_1
	fill_sequence("seg1108", &sequences[481], seg1110, intimidate_him, hit_him, NULL, seg1109, seg1110, NULL, 32792, 37167, 35208); // seg1108
	fill_sequence("seg1117", &sequences[482], seg1118_1122, accept, reject, NULL, seg1123_1125, seg1118_1122, NULL, 27125, 32917, 32417); // seg1117
	fill_sequence("seg1118_1122", &sequences[483], seg1068_1_1068_3_alt, go_home, go_to_the_tchois, NULL, seg1083_1084_alt, seg1068_1_1068_3_alt, NULL, 123917, 127917, 127042); // seg1118_1122
	fill_sequence("seg1083_1084_alt", &sequences[484], endcredits, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1083_1084_alt (END OF EPISODE -> END)
	fill_sequence("seg1068_1_1068_3_alt", &sequences[485], endcredits, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1068_1_1068_3_alt (END OF EPISODE -> END)
	fill_sequence("seg1110", &sequences[486], seg1111, let_him, open_yourself, NULL, seg1111, seg1112, NULL, 32542, 38500, 38500); // seg1110
	fill_sequence("seg1111", &sequences[487], eval_beat_up, shoot_him, dont, NULL, seg1113, seg1112_2, NULL, 9250, 12292, 12292); // seg1111
	fill_sequence("seg1113", &sequences[488], seg1114_a, surrender, fight, NULL, seg1114_b, seg1114_a, NULL, 96667, 99417, 96667); // seg1113
	fill_sequence("seg1112_2", &sequences[489], seg1113, let_him_alive, shoot_him, NULL, seg1112_1, seg1113, NULL, 13750, 17292, 16625); // seg1112_2
	fill_sequence("seg1114_a", &sequences[490], seg1115, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1114_a
	fill_sequence("seg1114_b", &sequences[491], seg1115, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1114_b
	fill_sequence("seg1115", &sequences[492], seg1116_a, tell_him, fuck_you, NULL, seg1116_b, seg1116_a, NULL, 16250, 20750, 18292); // seg1115
	fill_sequence("seg1116_a", &sequences[493], endcredits, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1116_a (END OF EPISODE -> END)
	fill_sequence("seg1116_b", &sequences[494], endcredits, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1116_b (END OF EPISODE -> END)
	fill_sequence("seg1109", &sequences[495], seg1106, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1109
	fill_sequence("seg1112", &sequences[496], seg1113, let_him_alive, shoot_him, NULL, seg1112_1, seg1113, NULL, 36792, 40333, 39375); // seg1112
	fill_sequence("seg1112_1", &sequences[497], seg1117_1, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1112_1
	fill_sequence("seg1123_1125", &sequences[498], eval_from_ep7_to_ep11, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1123_1125
	fill_sequence("seg1126_2", &sequences[499], seg1127_1129, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1126_2
	fill_sequence("seg1126_1", &sequences[500], seg1127_1129, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1126_1
	fill_sequence("seg1127_1129", &sequences[501], seg1131_1_1131_2, yes, no, NULL, seg1130_1130_1, seg1131_1_1131_2, NULL, 150708, 155250, 153375); // seg1127_1129
	fill_sequence("seg1130_1130_1", &sequences[502], endcredits, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1130_1130_1 (END OF EPISODE -> END)
	fill_sequence("seg1131_1_1131_2", &sequences[503], endcredits, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // seg1131_1_1131_2 (END OF EPISODE -> END)
	// END
	fill_sequence("endcredits", &sequences[504], NULL, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0); // endcredits
...
void game_post_run() {
	// Force 'Your Decisions' screen
	force_decisions = 1;
	
	// Set run_finished to 1 and update taken_decisions with last episode values
	global_vars.run_finished = 1;
	global_vars.taken_decisions += choices_num;
	choices_num = 0;
	
	// Delete completed run save and update global save
	sceIoRemove(PLAYTHROUGH_SAVE_FILE);
	FILE *f = fopen(GLOBAL_SAVE_FILE, "w");
	...
	while (!out) {
		if (draw_video_frame()) {
			...
			if (in_options) {
				...
				sprintf(str, "%u / 14", num_episodes_seen);
				draw_centered_text(270, str, 0.7f);
				draw_progressbar(352, 290, 256, 6, (float)num_episodes_seen / 14.0f, "##barchapters");
